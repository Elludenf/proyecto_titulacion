...
class Escuela extends CI_Controller
{
    ...
    function buscarEscuela()
    {
        ...
        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['escuelas'] = $this->Escuela_model->getEscuelaBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Escuela_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/escuelas/buscarEscuela';
