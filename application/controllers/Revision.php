...
class Revision extends CI_Controller
{
    ...
    function buscarRevision()
    {
        ...
        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['revisiones'] = $this->Revision_model->getRevisionBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Revision_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/revision/buscarRevision';
