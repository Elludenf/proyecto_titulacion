...
class Materia_x_plan_de_estudio extends CI_Controller
{
    ...
    function buscarMatXPlan()
    {

        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['materia_x_plan_de_estudio'] = $this->Materia_x_plan_de_estudio_model->getMatXPlanBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Materia_x_plan_de_estudio_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/materia_x_plan_de_estudio/buscarMatXPlan';
