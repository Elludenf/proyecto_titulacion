...
class Elabora extends CI_Controller
{
    ...
    function buscarElabora()
    {

        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['elabora'] = $this->Elabora_model->getElaboraBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Elabora_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/elabora/buscarElabora';
