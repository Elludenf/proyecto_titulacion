...
class Revdir_x_disertacion extends CI_Controller
{
    ...
    function buscarRevDirXDis()
    {

        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['revdir_x_disertacion'] = $this->Revdir_x_disertacion_model->getRevDirXDisBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Revdir_x_disertacion_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/revdir_x_disertacion/buscarRevDirXDis';
