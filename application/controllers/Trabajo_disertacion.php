...
class Trabajo_disertacion extends CI_Controller
{
    ...
    function buscarDisertacion()
    {

        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['trabajo_disertacion'] = $this->Trabajo_disertacion_model->getDisertacionBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Trabajo_disertacion_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/trabajo_disertacion/buscarDisertacion';
