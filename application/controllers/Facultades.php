...
class Facultades extends CI_Controller
{
    ...
    function buscarFacultad()
    {
        ...
        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['facultades'] = $this->Facultades_model->getFacultadBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Facultades_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/facultades/buscarFacultad';
