...
class Dicta extends CI_Controller
{
    ...
    function buscarDicta()
    {
        ...
        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['dicta'] = $this->Dicta_model->getDictaBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Dicta_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/dicta/buscarDicta';
