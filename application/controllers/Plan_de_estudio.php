...
class Plan_de_estudio extends CI_Controller
{
    ...
    function buscarPlanEstudio()
    {

        if (isset($_SERVER['HTTP_REFERER'])) {
            $data['planes_de_estudio'] = $this->Plan_de_estudio_model->getPlanEstudioBusqueda($this->input->post('search'));

            /*Empiezo de paginacion*/
            $total_rows = $this->Plan_de_estudio_model->countParamSearch($this->input->post('search'));

            $this->load->library('pagination');
            //   $config['total_rows'] = $total_rows;
            $config['per_page'] = 5;
            $config['uri_segment'] = 3;
            $config['base_url'] = base_url() . '/plan_de_estudio/buscarPlanEstudio';
